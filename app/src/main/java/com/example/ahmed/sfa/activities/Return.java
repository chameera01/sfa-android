...
public class Return extends AppCompatActivity implements SummaryUpdateListner {
    ...
    TextView subTotal, returnQty, discount, total, shelf, order, free;
...
//        getRepAndDeviceId();
    ...
    private void init() {
        ...
        if (pm.checkForLocationPermission()) {
            ...
            if (lastKnownLocation != null) {
                ...
                shelf = (TextView) findViewById(R.id.shelf_ri);
                order = (TextView) findViewById(R.id.order_ri);
                free = (TextView) findViewById(R.id.free_ri);


                final Button refresh = (Button) findViewById(R.id.refresh_ri);
                        ...
                        new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                if (rtotal > invTotal) {
                                    ...
                                    Intent intent = new Intent(Return.this, ReturnSummary.class);
                                    intent.putParcelableArrayListExtra("SIM_ARRAYLIST", list);
                                    intent.putExtra("RETURN_SUMMARY", payment);
    ...
    private double getReturnTotal() {
        SalesInvoiceSummary sim = SalesInvoiceSummary.createSalesInvoiceSummary(dbAdapter.getInvoicedItems());
        double total = sim.getTotal();
...
//        payment = new SalesPayment(getSalesInvoiceSummary(list));
//
//        Log.d("INVOICE", "inside submit- invoicedQty: " + payment.getInvQty());
//        Log.d("INVOICE", "inside submit- ReturnQty: " + payment.getReturnQty());
//
...
//                                        ManualSyncFromOtherActivities msofa = new ManualSyncFromOtherActivities(Return.this);
//                                        Boolean status = msofa.uploadSalesDetails(repId);
//                                    } else {
...
//        updateSummaryView(getSalesInvoiceSummary(dbAdapter.getInvoicedItems()));
    ...
    class DBAdapter extends BaseDBAdapter {
        ...
        public boolean insertIntoSalesReturnHeader(SalesPayment returnSummary, Location loc, String itineraryId, String customerNO, int onInvoiceReturnNo, boolean callingAct) {

            Log.d("NUM", "inside return insert method");
            openDB();

            ContentValues cv = new ContentValues();
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[0], itineraryId);
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[1], customerNO);
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[2], onInvoiceReturnNo);
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[3], DateManager.dateWithTimeToday());
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[4], getPaymentType(returnSummary));
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[5], returnSummary.getSubTotal());
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[6], returnSummary.getTotal());
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[7], returnSummary.getFullInvDisc());
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[8], returnSummary.getDiscount());
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[9], "PD"); //since return discounts do not include brand or full invoice discounts

            //if the activity launched from invoice activity this will be positive, negative otherwise
            //edit

            Log.d("INVOICE", "calling act: " + callingAct);
            if (callingAct) {
            ...
            Log.d("INVOICE", "ret qty: " + returnSummary.getInvQty());

            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[11], Constants.ACTIVE);
            cv.put(Constants.SALES_RETURN_TABLE_COLUMNS[12], returnSummary.getInvQty());
        ...
        public List<SalesInvoiceModel> getPrincipleQty(String principle) {

            Log.d("SCROLL", "inside getPrincipleQty");
            ArrayList<SalesInvoiceModel> data = new ArrayList<>();
            openDB();
            String sql = "SELECT * from temp_return WHERE returnQty !=0 AND PrincipleID = (SELECT PrincipleID FROM Mst_SupplierTable WHERE Principle = '" + principle + "')";
            ...
            while (cursor.moveToNext()) {
                try {
                    ...
                    Log.d("CHECK", ex.getLocalizedMessage());
                    Toast.makeText(getApplicationContext(), "Wrong Value ", Toast.LENGTH_SHORT).show();
                }
            }
            cursor.close();
